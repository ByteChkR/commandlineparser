    ...
    internal class LocalizableAttributeProperty
    {
        ...
        private string GetLocalizedValue()
        {
            ...
            if (_localizationPropertyInfo == null)
            {
                ...
                {
                    throw new
                        ArgumentException($"Invalid resource type '{_type.FullName}'! {_type.Name} is not visible for the parser! Change resources 'Access Modifier' to 'Public'",
                                          _propertyName
                                         );
                }

                PropertyInfo propertyInfo =
                    _type.GetProperty(_value, BindingFlags.Public | BindingFlags.GetProperty | BindingFlags.Static);

                if (propertyInfo == null ||
                    !propertyInfo.CanRead ||
                    (propertyInfo.PropertyType != typeof(string) && !propertyInfo.PropertyType.CanCast<string>()))
                {
            ...
            return _localizationPropertyInfo.GetValue(null, null)
                                            .Cast<string>();
        }
    }
}
