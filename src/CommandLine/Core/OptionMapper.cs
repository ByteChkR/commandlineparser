                ...
                StringComparer comparer)
        {
            IEnumerable<Tuple<SpecificationProperty, Maybe<Error>>> sequencesAndErrors = propertyTuples
                .Select(
                    pt =>
                    {
                        Maybe<IEnumerable<KeyValuePair<string, IEnumerable<string>>>> matched = options.Where(
                            s =>
                                s.Key.MatchName(((OptionSpecification)pt.Specification).ShortName, ((OptionSpecification)pt.Specification).LongName, comparer)
                        ).ToMaybe();
                        if (matched.IsJust())
                        {
                            IEnumerable<KeyValuePair<string, IEnumerable<string>>> matches = matched.GetValueOrDefault(Enumerable.Empty<KeyValuePair<string, IEnumerable<string>>>());
